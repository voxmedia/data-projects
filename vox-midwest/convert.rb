...
def generate_calculated_csv
  ...
  # make a csv of each submission and their states selected
  calculated_csv = CSV.open('data/calculated.csv', 'w', @csv_format)
  calculated_csv << ['state','count']
  calculated = JSON.parse( IO.read('calculated.json') )
  @valid_states.each do | state |
    calculated_csv << [ state, calculated[ state ] ]
  end
  calculated_csv.close()
end

generate_submissions_csvs()
generate_calculated_csv()
